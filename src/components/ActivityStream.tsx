...
export function ActivityStream({ projectId }: { projectId: string }) {
  ...
  const fetchActivities = async (silent = false) => {
    ...
    try {
      const res = await apiFetch(`/api/projects/${projectId}/activities`); if (res.ok) setActivities(await res.json());
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Recent changes in this project</p>
        </div>
        <button
          onClick={() => fetchActivities()}
          disabled={isLoading}
          className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-[#111111] px-2.5 py-1.5 rounded-lg border border-[#E8E8E8] hover:bg-[#F4F4F4] transition-colors"
        >
      ...
      {isLoading ? (
        ...
        <div className="py-10 text-center border-2 border-dashed border-[#E8E8E8] rounded-xl">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Actions will appear here as work progresses.</p>
        </div>
      ) : (
        <div className="flow-root max-h-80 overflow-y-auto">
          <ul className="-mb-6">
            {activities.map((a, idx) => {
              const meta = ACTION_META[a.action] ?? { icon: GitCommit, color: "text-slate-500 bg-[#F4F4F4]" };
